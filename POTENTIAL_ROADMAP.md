...
Allow a site owner to connect a public Mastodon account and display its public posts in Flightstead using the existing card designs.

### Possible behavior

- Follow the configured Mastodon account through ActivityPub.
- Store local snapshots so rendering a Flightstead page never depends on a live Mastodon request.
...
- Do not federate imported posts outward from Flightstead, avoiding loops and duplicate posts.
...
- Reconcile with Mastodon hourly and when the Flightstead server starts, recovering updates missed during downtime.
...
- Keep imported objects distinguishable from Flightstead-authored content for editing, deletion, feeds, search, and federation.
